...
void voiture::updateBatterie(float kw) {
    if ( capaciteBat * pourcentageBat + kw >= capaciteBat) pourcentageBat = 1 ;
    else  pourcentageBat = capaciteBat / (capaciteBat * pourcentageBat + kw) ;
...
istream& operator>>(istream& in, voiture& v) {
    if ( &in == &cin ) {
        cout << "\nImmat : " ; in >> v.immat  ;
        cout << "Marque : " ; in >> v.marque  ;
        cout << "Modele " ; in >> v.modele ;
        cout << "Capacite Batterie : "  ; in >> v.capaciteBat  ;
        cout << "Pourcentage Batterie : "  ; in >> v.pourcentageBat  ;
        cout << "Duree de vie : " ; in >> v.dureeVie  ;
    }
    else {
